...
# basic skat
skat <- function(Y, Y.hat, G, model, inference=T, X=NULL) {
...
quotient <- function(Y,                                                                                       # length-n vector of responses
  ...
  skat <- skat(Y, Y.hat, G, model, inference, X)
  ...
  if (inference & (model == 'gaussian')) {
    ...
    return(list('statistic'=skat$statistic / denom, 
                'variance'=skat$variance / denom^2, 
...
laxkat.quotient <- function(img,                                                                              # dataframe including response, covariates, and imaging data (one row for each sample)
